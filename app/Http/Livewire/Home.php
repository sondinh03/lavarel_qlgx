...
use App\Models\GradeLevel;       // ← thay Block (bảng: block → grade_levels)
use App\Models\NamHoc;
use App\Models\StudentNew;       // ← thay Student (bảng: student → students)
 ...
 *
 * ===================== MIGRATION MAP =====================
 *  Model cũ     │ Model mới        │ Bảng cũ  → Bảng mới
 * ──────────────┼──────────────────┼───────────────────────
 *  Lop          │ CatechismClass   │ lop      → classes
 *  Block        │ GradeLevel       │ block    → grade_levels
 *  Student      │ StudentNew       │ student  → students
 *  NamHoc       │ NamHoc           │ (không đổi)
 *
 *  Field cũ              │ Field mới
 * ───────────────────────┼──────────────────────
 *  lop.pid               │ classes.parish_id
 *  lop.schoolyear        │ classes.school_year_id
 *  lop.block             │ classes.grade_level_id
 *  lop.status (int 0/1)  │ classes.is_active (boolean)
 *  block.namhoc          │ grade_levels.school_year_id
 *  block.weight          │ grade_levels.sort_order
 *  student.sex           │ students.gender
 *  NamHoc.ofParish()     │ NamHoc.where('parish_id', ...)
 * =========================================================
 */
