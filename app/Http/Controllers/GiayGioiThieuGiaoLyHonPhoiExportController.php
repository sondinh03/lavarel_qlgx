...
class GiayGioiThieuGiaoLyHonPhoiExportController extends Controller
{
    public function store($slug, $id)
    {
        ...
        if($parishioners->deid != ''){
            ...
            $parishioners['giaohat'] = $deanery->name;
        }else{
            $parishioners['deid'] = '';
        ...
        $templateProcessor = new TemplateProcessor('word-template/GioiThieuGiaoLyHonPhoi.docx');
        ...
        $templateProcessor -> saveAs('GioiThieuGiaoLyHonPhoi.docx');
        return  response()->download('GioiThieuGiaoLyHonPhoi.docx')->deleteFileAfterSend(true);
    }
