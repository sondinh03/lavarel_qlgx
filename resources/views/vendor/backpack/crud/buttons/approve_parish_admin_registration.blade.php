@if ($entry->isPending())
    @if ($entry->createsNewParish())
        ...
        @endphp
        <button type="button" class="btn btn-sm btn-success" data-toggle="modal" data-target="#approveParishModal{{ $entry->getKey() }}">
            <i class="la la-check"></i> Duyệt
        </button>

        <div class="modal fade" id="approveParishModal{{ $entry->getKey() }}" tabindex="-1" role="dialog" aria-hidden="true">
