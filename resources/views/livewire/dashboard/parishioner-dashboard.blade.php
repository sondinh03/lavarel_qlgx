...
<div class="min-h-screen bg-apple-gray p-2 sm:p-4 lg:p-6" style="min-height: calc(100vh - 56px - var(--bottom-offset));">
    <div class="mx-auto max-w-7xl space-y-6">
            ...
            >
                <x-slot name="actions">
                    <a href="{{ route('parishioners.statistics') }}"
                        class="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-primary-600 bg-primary-50 hover:bg-primary-100 rounded-xl transition shadow-mac-sm">
        ...
        <a href="{{ route('parishioners.registrations.index') }}"
            class="flex items-start gap-3 p-4 rounded-2xl bg-amber-50/90 border border-amber-200/80 hover:bg-amber-100 transition shadow-mac-sm">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <x-stats.chart-card title="Tóm tắt nhanh">
                <div class="grid grid-cols-2 gap-3 text-sm">
                    <div class="rounded-xl bg-white/60 border border-black/[0.06] shadow-mac-sm p-3">
            ...
            <x-stats.chart-card title="Giáo dân mới" right="Gần đây">
                @if(count($recentParishioners) > 0)
                    <div class="-mx-6 -my-6 divide-y divide-black/[0.04]">
                        @foreach($recentParishioners as $p)
                        <div class="flex items-center justify-between px-6 py-3 hover:bg-black/[0.03] transition-colors duration-200">
                            <div class="min-w-0">
            ...
            <div class="space-y-5">
                <div>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        <a href="{{ route('parishioners.index') }}" class="px-4 py-3 rounded-2xl bg-white/60 hover:bg-black/[0.03] border border-black/[0.06] shadow-mac-sm transition">
                        ...
                        <a href="{{ route('parishioners.statistics') }}" class="px-4 py-3 rounded-2xl bg-primary-50 hover:bg-primary-100 border border-primary-200/80 shadow-mac-sm transition">
                        ...
                        @can('viewAny', \App\Models\ParishionerRegistrationRequest::class)
                        <a href="{{ route('parishioners.registrations.index') }}" class="px-4 py-3 rounded-2xl bg-amber-50 hover:bg-amber-100 border border-amber-200/80 shadow-mac-sm transition">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        <a href="{{ route('families.index') }}" class="px-4 py-3 rounded-2xl bg-white/60 hover:bg-black/[0.03] border border-black/[0.06] shadow-mac-sm transition">
                            <div class="text-sm font-semibold text-slate-800">Gia đình</div>
                            <div class="text-xs text-slate-400 mt-0.5">Sổ gia đình</div>
                        </a>
                        <a href="{{ route('marriage-announcements.index') }}" class="px-4 py-3 rounded-2xl bg-white/60 hover:bg-black/[0.03] border border-black/[0.06] shadow-mac-sm transition">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        <a href="{{ route('parish-group.index') }}" class="px-4 py-3 rounded-2xl bg-white/60 hover:bg-black/[0.03] border border-black/[0.06] shadow-mac-sm transition">
                            <div class="text-sm font-semibold text-slate-800">Giáo họ</div>
                            <div class="text-xs text-slate-400 mt-0.5">Quản lý giáo họ</div>
                        </a>
                        <a href="{{ route('associations.index') }}" class="px-4 py-3 rounded-2xl bg-white/60 hover:bg-black/[0.03] border border-black/[0.06] shadow-mac-sm transition">
                            <div class="text-sm font-semibold text-slate-800">Hội đoàn</div>
                            <div class="text-xs text-slate-400 mt-0.5">Quản lý hội đoàn</div>
                        </a>
                        <a href="{{ route('holy-names.index') }}" class="px-4 py-3 rounded-2xl bg-white/60 hover:bg-black/[0.03] border border-black/[0.06] shadow-mac-sm transition">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        <a href="{{ route('parishioners.register.public') }}" target="_blank" class="px-4 py-3 rounded-2xl bg-emerald-50 hover:bg-emerald-100 border border-emerald-200/80 shadow-mac-sm transition">
