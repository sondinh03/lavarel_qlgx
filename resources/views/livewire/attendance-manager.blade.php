        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div class="px-6 py-4 border-b border-slate-200 bg-slate-50/70">
                <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
                    ...
                    {{-- RIGHT: Actions — chỉ hiện khi đã chọn lớp --}}
                    @if($selectedClassId)
                            ...
                            <x-table-header class="text-center min-w-[120px]">
                                <div class="flex flex-col gap-1">
                                    ...
                                    <button
                                        ...
                                        wire:target="markAllPresent({{ $session['id'] }})"
                                        class="text-[9px] text-green-600 hover:text-green-700 hover:underline">
                                        <span wire:loading.remove wire:target="markAllPresent({{ $session['id'] }})">✓ Tất cả</span>
                                        <span wire:loading wire:target="markAllPresent({{ $session['id'] }})">⏳</span>
                                    </button>
                                    @endif
                                </div>
                            </x-table-header>
                        ...
                        <tr class="hover:bg-slate-50 transition-colors" wire:key="student-{{ $student->id }}">
                            ...
                            <td class="px-3 py-3 text-center">
                                ...
                                <div class="flex gap-1 justify-center">
                                    ...
                                    {{-- Vắng có phép — mở modal để nhập note --}}
                                    <div class="relative inline-block" x-data="{ open: false }">
                                        <button
                                            wire:click="openNoteModal({{ $student->id }}, {{ $session['id'] }})"
                                            @mouseenter="open = true"
                                            @mouseleave="open = false"
                                        ...
                                        {{-- Tooltip đọc note — chỉ cần có note, không cần check status --}}
                                        @if($note)
                                        <div x-show="open"
                                            x-transition
                                            x-cloak
                            ...
                            <tr wire:key="mobile-student-{{ $student->id }}"
                                ...
                                <td class="px-3 py-3">
                                    ...
                                    <div class="flex gap-1.5 justify-center">
                                        ...
                                        <button
                                            wire:click="openNoteModal({{ $student->id }}, {{ $currentSession['id'] ?? 0 }})"
                                            wire:loading.attr="disabled"
                                            wire:target="openNoteModal"
                ...
                <div class="lg:hidden fixed left-0 right-0 z-40
                    ...
                    <div class="flex items-center gap-3 max-w-7xl mx-auto">
                        ...
                        <button
                            ...
                            {{ empty($draftAttendance) ? 'disabled' : '' }}
...
<div
    ...
    <div
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-5">
            ...
            {{-- Note Input — Alpine đếm ký tự realtime, không cần Livewire round-trip --}}
            ...
            {{-- Quick Reasons — dùng $wire.set để update ngay không cần round-trip --}}
            <div>
