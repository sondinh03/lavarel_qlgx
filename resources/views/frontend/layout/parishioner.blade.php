...
<head>
    ...
    <script>
        (function() {
            ...
            var mini = localStorage.getItem('parishionerSidebarMini') === 'true';
    ...
    <style>
        ...
        /* Transition chỉ khi user toggle */
...
<body class="min-h-screen bg-apple-gray text-slate-800 antialiased"
    x-data="{
        ...
        openGroups: (function() {
            ...
            const ls = JSON.parse(localStorage.getItem('parishionerOpenGroups') || '[]');
            return ls;
        })(),
        ...
        toggleGroup(name) {
            ...
            localStorage.setItem('parishionerOpenGroups', JSON.stringify(this.openGroups));
        }
    }"
    ...
    <aside id="sidebar"
        ...
        <div class="flex items-center gap-3 px-4 h-16 border-b border-slate-100 flex-shrink-0">
            {{-- Link về trang chọn module --}}
            <a href="{{ route('module.select') }}" class="flex items-center gap-3 min-w-0">
            ...
            <button @click="
                    ...
                    localStorage.setItem('parishionerSidebarMini', nextMini);
        ...
        <nav id="sidebar-nav" class="flex-1 overflow-y-auto overflow-x-hidden min-h-0 py-3 px-2 space-y-0.5">
            ...
            <div class="relative has-flyout">
                <button @click="toggleGroup('parishioners')"
                    class="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition group
                        {{ $isParishionersActive ? 'text-primary-700 font-semibold' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900' }}">
                    <svg class="w-5 h-5 flex-shrink-0
                        {{ $isParishionersActive ? 'text-primary-600' : 'text-slate-400 group-hover:text-slate-600' }}"
                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                    </svg>
                    <span class="sidebar-label flex-1 text-left truncate">Giáo Dân</span>
                    <svg class="sidebar-chevron w-3.5 h-3.5 flex-shrink-0 text-slate-400 transition-transform duration-200"
                        :class="openGroups.includes('parishioners') ? 'rotate-180 text-primary-600' : 'text-slate-400'"
                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                    </svg>
                    @if($isParishionersActive)
                    <span class="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-6 bg-primary-500 rounded-r-full"></span>
                    @endif
                </button>

                {{-- Accordion submenu --}}
                <div x-show="openGroups.includes('parishioners') && !sidebarMini"
                    style="{{ $isParishionersActive ? '' : 'display:none' }}"
            ...
            <div class="pt-2 mt-2 border-t border-slate-100">
                <a href="{{ auth()->user()->usesCatechistLayout() ? route('catechist.dashboard') : route('parish-admin.dashboard') }}"
                    ...
                    <span class="sidebar-label truncate">Sang phân hệ Giáo lý</span>
                </a>
            </div>
            @endif
    ...
    <div id="main-wrapper"
        ...
        x-init="
            localStorage.setItem('parishionerOpenGroups', JSON.stringify(openGroups));

            $watch('openGroups', value => {
                localStorage.setItem('parishionerOpenGroups', JSON.stringify(value));
            });
