    		...
    		<div class="shadow-sm border bg-white p-3 mb-3">
    			<div class="row">
    				<div class="form-group col-6 col-md-3 mb-3">
    					<label class="mb-1">Chọn bí tích</label>
    					<select class="form-control" name="bitich">
    						<option></option>
    						@foreach($bitich as $key => $item)
    							@if(!empty($_GET)) {
        							@if($key == $_GET['bitich'])
    				...
    				<div class="form-group col-6 col-md-3 mb-3">
    					...
    					<input class="form-control time_start" required="required" name="time_start" value="<?php if(!empty($_GET['time_start'])){ echo $_GET['time_start']; }?>">
    				</div>
    				<div class="form-group col-6 col-md-3 mb-3">
    					<label class="mb-1">Thời gian kết thúc</label>
    					<input class="form-control time_stop" required="required" name="time_stop" value="<?php if(!empty($_GET['time_stop'])){ echo $_GET['time_stop']; } ?>">
    				</div>
    				@if(!empty($form))
		...
		<div class="shadow border border-light-subtle bg-white rounded-3">	
			<div class="card">
				<div class="card-header border-light-subtle bg-white">
					<h3 class="card-title fs-6 fw-semibold py-2 mb-0">Danh sách bí tích</h3>
				</div>
				<div class="card-body">
    					...
    					<table class="table table-vcenter text-nowrap mb-0 table-striped table-bordered border-top">
                          	<thead>
                            	<tr>
                                  	...
                                  	<th scope="col" class="fw-medium">Ghi chú</th>
                                </tr>
                          	</thead>
                          	<tbody>
                          		@forelse($parishioners as $key => $item)
                          			<tr>
                                      	...
                                      	<td class="border-top-0 border-bottom-0">{{$item->residence}}, {{$item->ward}}, {{$item->province}}</td>
                                      	<td class="border-top-0 border-bottom-0">{{$item->note}}</td>
                                    </tr>
                                @empty
                                	<tr>
                                		<td colspan="9">
