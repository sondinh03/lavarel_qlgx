		...
		<div class="shadow border border-light-subtle bg-white rounded-3">	
			<div class="card">
				<div class="card-header border-light-subtle bg-white">
					<h3 class="card-title fs-6 fw-semibold py-2 mb-0">Danh sách rao hôn phối</h3>
				</div>
				<div class="card-body">
    					...
    					<table class="table table-vcenter text-nowrap mb-0 table-striped table-bordered border-top">
                          	<thead>
                            	<tr>
                                  	...
                                  	<th scope="col" class="fw-medium">Đôi rao</th>
                                  	<th scope="col" class="fw-medium">Nam</th>
                                  	<th scope="col" class="fw-medium">Nữ</th>
                                  	<th scope="col" class="fw-medium">Linh mục chứng</th>
                                  	<th scope="col" class="fw-medium">Rao lần 1</th>
                                  	<th scope="col" class="fw-medium">Rao lần 2</th>
                                  	<th scope="col" class="fw-medium">Rao lần 3</th>
                                </tr>
                          	</thead>
                      		<tbody>
                          		@forelse($marriageannouncement as $key => $item)
                          			<tr>
                                      	<th scope="row">{{$key+1}}</th>
                                      	<td class="border-top-0 border-bottom-0"><a class="text-decoration-none" href="{{$item->slug}}" title="{{$item->name}}">{{$item->name}}</a></td>
                                      	<td class="border-top-0 border-bottom-0">{{$item->nam}}</td>
                                      	<td class="border-top-0 border-bottom-0">{{$item->nu}}</td>
                                      	<td class="border-top-0 border-bottom-0">{{$item->priest}}</td>
                                      	<td class="border-top-0 border-bottom-0">{{$item->announcements_one}}</td>
                                      	<td class="border-top-0 border-bottom-0">{{$item->announcements_two}}</td>
                                      	<td class="border-top-0 border-bottom-0">{{$item->announcements_three}}</td>
                                  	</tr>
                          		@empty
                                	<tr>
                                		<td colspan="8">
